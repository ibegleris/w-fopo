...
def RK5mm(dAdzmm,u1,dz,M1,M2,t,n2,lamda,tsh,w,woffset,dt,hf):
   ...
   #
    ...
    #p.enable()
    A1 = dz*dAdzmm(u1,                                                                                                  M1,M2,t,n2,lamda,tsh,w,woffset,dt,hf)
    A2 = dz*dAdzmm(u1 + (1./5)*A1,                                                                                      M1,M2,t,n2,lamda,tsh,w,woffset,dt,hf)
    A3 = dz*dAdzmm(u1 + (3./40)*A1       + (9./40)*A2,                                                                  M1,M2,t,n2,lamda,tsh,w,woffset,dt,hf)
    A4 = dz*dAdzmm(u1 + (3./10)*A1       - (9./10)*A2       + (6./5)*A3,                                                M1,M2,t,n2,lamda,tsh,w,woffset,dt,hf)
    A5 = dz*dAdzmm(u1 - (11./54)*A1      + (5./2)*A2        - (70./27)*A3      + (35./27)*A4,                           M1,M2,t,n2,lamda,tsh,w,woffset,dt,hf)
    A6 = dz*dAdzmm(u1 + (1631./55296)*A1 + (175./512)*A2    + (575./13824)*A3  + (44275./110592)*A4 + (253./4096)*A5,   M1,M2,t,n2,lamda,tsh,w,woffset,dt,hf)
...
def RK5mm(dAdzmm,u1,dz,M1,M2,t,n2,lamda,tsh,w,woffset,dt,hf):
    ...
    A2 = dz*dAdzmm(u1 + (1./5)*A1,                                                                                                          M1,M2,t,n2,lamda,tsh,w,woffset,dt,hf)
    A3 = dz*dAdzmm(u1 + (3./40)*A1       + (9./40)*A2,                                                                                      M1,M2,t,n2,lamda,tsh,w,woffset,dt,hf)
    A4 = dz*dAdzmm(u1 + (44./45)*A1      - (56./15)*A2        + (32./9)*A3,                                                                 M1,M2,t,n2,lamda,tsh,w,woffset,dt,hf)
    A5 = dz*dAdzmm(u1 + (19372./6561)*A1 - (25360./2187)*A2   + (64448./6561)*A3      - (212./729)*A4,                                      M1,M2,t,n2,lamda,tsh,w,woffset,dt,hf)
    A6 = dz*dAdzmm(u1 + (9017./3168)*A1  - (355./33)*A2       + (46732./5247)*A3      + (49./176)*A4   - (5103./18656)*A5,                  M1,M2,t,n2,lamda,tsh,w,woffset,dt,hf)
...
def dAdzmm_roff_s0(u0,M1,M2,t,n2,lamda,tsh,w,woffset,dt,hf):
    """
...
def dAdzmm_roff_s1(u0,M1,M2,t,n2,lamda,tsh,w,woffset,dt,hf):
    """
    calculates the nonlinear operator for a given field u0
    use: dA = dAdzmm(u0)
    """
    ...
    N = -1j*n2*2*pi/lamda*(N + tsh*imfft((np.tile(w,(len(u0[0,:]),1)).T + woffset)*mfft(N)))

    return N


def dAdzmm_ron_s0(u0,M1,M2,t,n2,lamda,tsh,w,woffset,dt,hf):
...
def dAdzmm_ron_s1(u0,M1,M2,t,n2,lamda,tsh,w,woffset,dt,hf):
    fr = 0.18
